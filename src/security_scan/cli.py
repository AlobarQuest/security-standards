...
from security_scan import allowlist as allowlist_mod
from security_scan import manifest, predicates, repo
...
def _judgment_finding(rule) -> Finding:
    return Finding(
        ...
        file=None,
        line=None,
        evidence="requires agent judgment",
        remediation=rule["remediation"],
        reason=rule["reason"],
        kind="judgment",
    )
...
def scan(repo_path, cache_path=_DEFAULT_CACHE, category="security") -> tuple[dict, int]:
    ...
    if not repo.is_git_repo(repo_path):
        ...
        by_sev["BLOCK"] = 1
        report = {
            "meta": {"rules_source": "none", "rules_evaluated": 0, "allowlisted": 0},
            "summary": {"by_severity": by_sev, "total": 1},
            "findings": [err.to_dict()],
            "allowlisted": [],
        }
        return report, 1
    repo_path = repo.root(repo_path) or repo_path
    ...
    for rule in rules:
        ...
        if kind in ("manifest_present", "manifest_matches_usage"):
            ...
            # BLOCKed for lacking *.env/*.key patterns it has no use for
            # (avoids false-positive gates).
            if uses_bws:
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(
        description="Scan a repo against security standards (infra-brain)."
    )
