...
def _f(rule="bws.r", file="a.py", evidence="ev"):
    return Finding(rule_id=rule, severity=Severity.BLOCK, file=file, line=1,
                   evidence=evidence, remediation="x", reason="y", kind="deterministic")
