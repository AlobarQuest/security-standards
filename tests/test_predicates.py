...
def test_forbidden_pattern_history_scope(git_repo):
    ...
    git_repo.commit()
    rule = _rule(
        {"kind": "forbidden_pattern", "pattern": r"0\.[0-9a-f-]{36}\.", "scope": "history"}
    )
    findings = predicates.evaluate(rule, git_repo.path)
    assert len(findings) >= 1
...
def test_clean_repo_yields_nothing(git_repo):
    ...
    git_repo.commit()
    rule = _rule(
        {"kind": "forbidden_pattern", "pattern": r"0\.[0-9a-f-]{36}\.", "scope": "tracked"}
    )
...
def test_required_pattern_passes_when_present(git_repo):
    ...
    git_repo.commit()
    rule = _rule(
        {"kind": "required_pattern", "pattern": "fetch_bws_secret", "globs": ["*.sh"]},
